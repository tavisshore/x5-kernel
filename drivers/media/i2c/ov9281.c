...
#define OV9281_EXPOSURE_DEFAULT		0x0282
...
#define OV9281_REG_DGAIN		0x350a
#define OV9281_DGAIN_MIN		0x100
#define OV9281_DGAIN_MAX		0xfff
#define OV9281_DGAIN_STEP		1
#define OV9281_DGAIN_DEFAULT		0x100
...
#define OV9281_TEST_PATTERN_BAR_SHIFT	2

/* Mirror/Flip registers */
#define OV9281_REG_TIMING_FORMAT1	0x3820
#define OV9281_REG_TIMING_FORMAT2	0x3821
 ...
 * @supplies: Power supply regulators
 ...
 * @hflip_ctrl: Pointer to horizontal flip control
 * @vflip_ctrl: Pointer to vertical flip control
 * @dgain_ctrl: Pointer to digital gain control
 * @exp_ctrl: Pointer to exposure control
 * @again_ctrl: Pointer to analog gain control
 * @vblank: Vertical blanking in lines
 * @cur_mbus_index: Current media bus format index
...
static const u32 ov9281_mbus_codes[] = {
...
static const char * const ov9281_test_pattern_menu[] = {
	...
	"Vertical Color Bar",
	"Color Bar Type 2",
	"Color Bar Fade to Gray Type 1",
	"Color Bar Fade to Gray Type 2",
};

/* Sensor mode registers for 1280x800 (full resolution) */
...
/* Sensor mode registers for 1280x720 (cropped) */
static const struct ov9281_reg mode_1280x720_regs[] = {
	{0x0302, 0x32},
	{0x030d, 0x50},
	{0x030e, 0x02},
	{0x3001, 0x00},
...
/* Sensor mode registers for 640x400 (2x2 binned) */
static const struct ov9281_reg mode_640x400_regs[] = {
	{0x0302, 0x32},
	{0x030d, 0x50},
	{0x030e, 0x02},
	{0x3001, 0x00},
...
static const struct ov9281_mode supported_modes[] = {
	{
		/* Mode 0: 1280x800 full resolution */
		.width = 1280,
		.height = 800,
		.hblank = 250,
		.vblank = 942,
 ...
 * Supported controls:
 * - V4L2_CID_VBLANK
 * - V4L2_CID_EXPOSURE (cluster with V4L2_CID_ANALOGUE_GAIN)
...
static int ov9281_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_DIGITAL_GAIN:
		...
		ret = ov9281_write_reg(ov9281, OV9281_REG_DGAIN, 2, ctrl->val);
	...
	case V4L2_CID_TEST_PATTERN:
		...
		if (ctrl->val)
			val = (ctrl->val - 1) << OV9281_TEST_PATTERN_BAR_SHIFT |
			      OV9281_TEST_PATTERN_ENABLE;
		else
			val = 0;
		ret = ov9281_write_reg(ov9281, OV9281_REG_TEST_PATTERN, 1, val);
	...
	case V4L2_CID_HFLIP:
		...
		ret = ov9281_read_reg(ov9281, OV9281_REG_TIMING_FORMAT2,
				      1, &val);
		if (!ret) {
			if (ctrl->val)
				val |= OV9281_FLIP_BIT;
				...
				val &= ~OV9281_FLIP_BIT;
			ret = ov9281_write_reg(ov9281,
					       OV9281_REG_TIMING_FORMAT2,
					       1, val);
		}
	...
	case V4L2_CID_VFLIP:
		...
		ret = ov9281_read_reg(ov9281, OV9281_REG_TIMING_FORMAT1,
				      1, &val);
		if (!ret) {
			if (ctrl->val)
				val |= OV9281_FLIP_BIT;
...
{
	if (code->index >= ARRAY_SIZE(ov9281_mbus_codes))
		return -EINVAL;

	code->code = ov9281_mbus_codes[code->index];
				  ...
				  struct v4l2_subdev_frame_size_enum *fsize)
{
	...
	for (i = 0; i < ARRAY_SIZE(ov9281_mbus_codes); i++)
		if (fsize->code == ov9281_mbus_codes[i])
			break;

	if (i == ARRAY_SIZE(ov9281_mbus_codes))
		return -EINVAL;
				   ...
				   struct v4l2_subdev_format *fmt)
{
	...
	fmt->format.code = ov9281_mbus_codes[ov9281->cur_mbus_index];
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	/* Validate requested mbus code; default to index 0 if unknown */
	for (i = 0; i < ARRAY_SIZE(ov9281_mbus_codes); i++)
		if (fmt->format.code == ov9281_mbus_codes[i])
			break;

	if (i == ARRAY_SIZE(ov9281_mbus_codes))
		i = 0;

	ov9281->cur_mbus_index = i;
			       ...
			       struct v4l2_subdev_state *sd_state)
{
	...
	ov9281_fill_pad_format(ov9281, &supported_modes[0], &fmt);
...
static int ov9281_parse_hw_config(struct ov9281 *ov9281)
{
	...
	ret = devm_regulator_bulk_get(ov9281->dev, OV9281_NUM_SUPPLIES,
				      ov9281->supplies);
	if (ret) {
		dev_err(ov9281->dev, "failed to get regulators");
		return ret;
	}
...
static int ov9281_power_on(struct device *dev)
{
	...
	if (ret) {
		dev_err(ov9281->dev, "failed to enable regulators");
		return ret;
	}
...
static int ov9281_init_controls(struct ov9281 *ov9281)
{
	...
	/* Digital gain */
	ov9281->dgain_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
					       &ov9281_ctrl_ops,
					       V4L2_CID_DIGITAL_GAIN,
					       OV9281_DGAIN_MIN,
					       OV9281_DGAIN_MAX,
					       OV9281_DGAIN_STEP,
					       OV9281_DGAIN_DEFAULT);
	...
	/* Test pattern */
	ov9281->test_pattern_ctrl = v4l2_ctrl_new_std_menu_items(ctrl_hdlr,
					&ov9281_ctrl_ops,
					V4L2_CID_TEST_PATTERN,
	...
	/* Flip controls */
	ov9281->hflip_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
					       &ov9281_ctrl_ops,
					       V4L2_CID_HFLIP,
					       0, 1, 1, 0);

	ov9281->vflip_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
					       &ov9281_ctrl_ops,
					       V4L2_CID_VFLIP,
					       0, 1, 1, 0);
